...
function showWarning(message) {
    if (/session|authentication required|unauthorized|forbidden|too many requests|429|log in again/i.test(String(message || ''))) {
        return;
    }
...
async function loadFeedback() {
    try {
        ...
        const feedbacks = Array.isArray(payload) ? payload : (Array.isArray(payload.data) ? payload.data : []);
...
function createFeedbackCard(feedback) {
    ...
    headerInfo.className = 'feedback-header-info';
    const clientName = document.createElement('div');
    clientName.className = 'client-name';
    clientName.textContent = feedback.client_name;
    headerInfo.appendChild(clientName)    
    const feedbackDate = document.createElement('div');
    feedbackDate.className = 'feedback-date';
    feedbackDate.textContent = new Date(feedback.created_at).toLocaleDateString();
    headerInfo.appendChild(feedbackDate);    
    const starsDiv = document.createElement('div');
    starsDiv.className = 'feedback-stars';
    starsDiv.innerHTML = stars;    
    const serviceDiv = document.createElement('div');
    serviceDiv.className = 'feedback-service';
    serviceDiv.textContent = feedback.service_category;    
    const commentDiv = document.createElement('div');
    commentDiv.className = 'feedback-comment';
    commentDiv.textContent = feedback.comment;    
...
async function safeFetch(path, options = {}) {
    ...
    try {
        ...
        // If server returns HTML instead of JSON (like a login page), fail gracefully.
...
async function loadProjects() {
    ...
    // Handle paginated data formats natively
    const data = Array.isArray(res) ? res : (res && Array.isArray(res.data) ? res.data : null);
    if (!data) {
        list.innerHTML = `<li class="project-item" style="color: var(--danger-red);">Failed to parse project data format.</li>`;
        return;
    }

    const countEl = document.getElementById("projectCount");
    if (countEl) countEl.textContent = `Active clients: ${data.length}`;
    ...
    data.forEach(p => {
        ...
        li.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                <div>
                    <h4 style="margin: 0; color: white;">${escapeHtml(p.title)}</h4>
                    ${deadlineHtml}
                </div>
                <span style="padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: bold; background: ${statusColor}22; color: ${statusColor}; border: 1px solid ${statusColor}55;">${escapeHtml(p.status)}</span>
            </div>
            <div style="color: #8892b0; font-size: 13px; margin-bottom: 10px;">${escapeHtml(p.desc || "System parameters initialized.")}</div>
...
async function loadClients() {
    ...
    data.forEach(c => {
        ...
        li.innerHTML = `<strong style="color: #fff; font-size: 16px;">${escapeHtml(name)}</strong><br><small style="color: #4facfe;">${escapeHtml(subtitle)}</small>`;
...
async function refreshMessageThread() {
    ...
    // Fetch files
    const filesData = await safeFetch(`/api/messages/${currentClientId}/files`);
    const filesMap = {};
    if (filesData && filesData.files) {
        filesData.files.forEach(f => {
            if (!filesMap[f.message_id]) filesMap[f.message_id] = [];
            filesMap[f.message_id].push(f);
        });
    }
    ...
    box.innerHTML = data.map(m => {
        ...
        let filesHtml = '';
        if (filesMap[m.id] && filesMap[m.id].length > 0) {
            filesHtml = '<div style="margin-top: 10px; display: flex; flex-wrap: wrap; gap: 8px;">';
            filesMap[m.id].forEach(f => {
                const fileSizeKb = (f.file_size / 1024).toFixed(1);
                filesHtml += `
                    <a href="/api/files/${f.id}/download" 
                       style="padding: 6px 10px; background: rgba(0,242,254,0.2); border: 1px solid #00f2fe; border-radius: 6px; color: #00f2fe; text-decoration: none; font-size: 12px; display: flex; align-items: center; gap: 6px;">
...
async function uploadFile(fileInput) {
    ...
    // First, create a text message to attach the file to
    const fileNotifyMsg = `📎 File: ${file.name}`;
    
    const msgResponse = await safeFetch(`/api/messages/${currentClientId}`, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({ 
            content: fileNotifyMsg,
            sender: 'admin'
        })
    });

    if (!msgResponse || msgResponse.status !== "success") {
        showWarning("Failed to create message for file");
        return;
    }

    // Now get the latest message ID (we'll use the latest one as attachment)
    const messagesData = await safeFetch(`/api/messages/${currentClientId}`);
    if (!messagesData || !Array.isArray(messagesData) || messagesData.length === 0) {
        showWarning("Failed to get message for file attachment");
        return;
    }
    ...
    try {
        ...
        if (fileInput2) fileInput2.disabled = true;

        const response = await fetch(`/api/messages/${messageId}/upload`, {
            method: 'POST',
            body: formData
        });

        if (uploadBtn) uploadBtn.disabled = false;
        if (fileInput2) fileInput2.disabled = false;

        if (!response.ok) {
            const error = await response.json();
            showWarning(`Upload failed: ${error.error || 'Unknown error'}`);
            fileInput.value = '';
            return;
        }
        ...
        if (result.status === 'success') {
            showWarning(`✓ File "${file.name}" uploaded successfully!`);
        ...
        showWarning("Network error during upload");
        fileInput.value = '';
    }
}
...
async function addClient() {
    ...
    if (response && response.status === "success") {
        ...
        showWarning(`✓ Client "${name}" added successfully!`);
        await loadClients();
    } else {
